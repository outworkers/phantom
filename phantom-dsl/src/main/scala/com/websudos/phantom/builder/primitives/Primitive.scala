...
private[phantom] object DateSerializer {
  private[this] lazy val formatter = DateTimeFormat.forPattern("yyyy-mm-dd'T'HH:mm:ss")
  private[this] lazy val printer = ISODateTimeFormat.dateTime()

  def asCql(date: Date): String = {
    CQLQuery.empty.singleQuote(printer.print(new DateTime(date, DateTimeZone.UTC)))
  }

  def asCql(date: DateTime): String = {
    CQLQuery.empty.singleQuote(printer.print(date.toDateTime(DateTimeZone.UTC)))
  }
}

abstract class Primitive[RR] {

  protected[this] def nullCheck[T](column: String, row: Row)(fn: Row => T): Option[T] = {
    if (row.isNull(column)) None else Try(fn(row)).toOption
  }
...
trait DefaultPrimitives {
  ...
  implicit object DateTimeIsPrimitive extends Primitive[DateTime] {
    ...
    override def fromRow(column: String, row: Row): Option[DateTime] = nullCheck(column, row) {
      r => new DateTime(r.getDate(column), DateTimeZone.UTC)
    }
  ...
  implicit object InetAddressPrimitive extends Primitive[InetAddress] {
    ...
    override def asCql(value: InetAddress): String = CQLQuery.empty.singleQuote(value.toString)
