...
trait ExecutableQuery[T <: CassandraTable[T, _], R] extends CassandraResultSetOperations {
   ...
   * @return
   */
  def fetchEnumerator(implicit session: Session, ctx: scala.concurrent.ExecutionContext = Manager.scalaExecutor): ScalaFuture[PlayEnumerator[R]] = {
    future() map {
      resultSet => {
   ...
   * @return
   */
  def one(implicit session: Session, ctx: scala.concurrent.ExecutionContext = Manager.scalaExecutor): ScalaFuture[Option[R]] = {
   ...
   * @return
   */
  def fetch(implicit session: Session, ctx: scala.concurrent.ExecutionContext = Manager.scalaExecutor): ScalaFuture[Seq[R]] = {
