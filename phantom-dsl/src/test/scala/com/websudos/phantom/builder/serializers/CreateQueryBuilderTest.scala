  ...
  "The CREATE query builder" - {
    "should allow specifying table creation options" - {
      ...
      "serialise a simple create query with a SizeTieredCompactionStrategy and no compaction strategy options set" in {
        ...
        qb shouldEqual """CREATE TABLE phantom."BasicTable" (id uuid, id2 uuid, id3 uuid, placeholder text, PRIMARY KEY (id, id2, id3)) WITH compaction = { 'class' : 'SizeTieredCompactionStrategy' }"""
      }
      ...
      "serialise a simple create query with a SizeTieredCompactionStrategy and 1 compaction strategy options set" in {
        ...
        qb shouldEqual """CREATE TABLE phantom."BasicTable" (id uuid, id2 uuid, id3 uuid, placeholder text, PRIMARY KEY (id, id2, id3)) WITH compaction = { 'class' : 'SizeTieredCompactionStrategy', 'sstable_size_in_mb' : '50.0 MiB' }"""
      }
      ...
      "allow specifying a bloom_filter_fp_chance using a Double param value" in {
        ...
        qb shouldEqual """CREATE TABLE phantom."BasicTable" (id uuid, id2 uuid, id3 uuid, placeholder text, PRIMARY KEY (id, id2, id3)) WITH bloom_filter_fp_chance = 5.0"""
      }
    }
